      ...
      for line in input_file:
        ...
        if attr == 'sampler' and 'bimpler' in globals():
          if data[0] in bimpler.IMPLEMENTED_SAMPLERS.keys():
            ...
            # --- FIX START: Handle bilby v2+ EntryPoints ---
            if hasattr(bimpler, 'get_sampler_class'):
                # Modern bilby API
                sampler_cls = bimpler.get_sampler_class(data[0])
                self.sampler_kwargs = sampler_cls.default_kwargs
            else:
                # Legacy bilby API
                self.sampler_kwargs = bimpler.IMPLEMENTED_SAMPLERS[data[0]].default_kwargs
            # --- FIX END ---
